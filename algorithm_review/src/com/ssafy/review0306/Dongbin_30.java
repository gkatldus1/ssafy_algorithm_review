package com.ssafy.review0306;

import java.util.*;

public class Dongbin_30 {
	
	int lowerBound(ArrayList<String> arr, String target, int start, int end) {
		while(start < end) {
			int mid = (start + end) / 2;
	...
	int upperBound(ArrayList<String> arr, String target, int start, int end) {
		while(start < end) {
			int mid = (start + end) / 2;
			if(arr.get(mid).compareTo(target) > 0)	end = mid;
			else start = mid + 1;
	...
	int countByRange(ArrayList<String> arr, String leftValue, String rightValue) {
	...
	public ArrayList<ArrayList<String>> reversedArr = new ArrayList<>();
