...
public class gabia01 {
	 ...
	 public static class Player implements Comparable<Player>{
		 ...
		 public int compareTo(Player o) {
			 if(this.score == o.score)	return Integer.compare(this.idx,  o.idx);
	 ...
	 public static int solution(int K, String[] user_scores) {
		 ...
		 for(int i=0; i<user_scores.length; i++) {
			 ...
			 if(size < K && size < players.size()) {
				 rank = (ArrayList<String>) players.stream().map(Player::getName).collect(Collectors.toList());
				 answer++;
				 continue;
			 }
			 ...
			 for(int j=0; j<Math.min(players.size(), K); j++) {
				 if(!rank.get(j).equals(players.get(j).getName())) {
					 answer++;
					 rank = (ArrayList<String>)players.stream().limit(K).map(Player::getName).collect(Collectors.toList());
					 break;
				 }
			 }
